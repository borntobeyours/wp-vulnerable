	...
	*/
    ...
    static function wsm_activate( $networkwide){
      ...
      self::wsmCreateDatabaseSchema();
		...
		$wpdb->query( 'Drop Index  IF EXISTS `index_type_hash` on '.$wpdb->base_prefix.'wsm_url_log');
		$wpdb->query( 'Drop Index  IF EXISTS `index_tb_hash` on '.$wpdb->base_prefix.'wsm_url_log');
		$wpdb->query( 'ALTER TABLE '.$wpdb->base_prefix.'wsm_url_log ADD INDEX index_hash (hash) ');
		$wpdb->query( 'ALTER TABLE '.$wpdb->base_prefix.'wsm_url_log ADD INDEX index_pageid (pageId) ');
